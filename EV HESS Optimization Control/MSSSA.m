...
function [Ibat,Ic,Eb,Euc,FoodFitness,FoodPosition,Convergence_curve]=MSSSA(N,Max_iter,lb,ub,dim,fobj,Edemand,SOCbat,SOCuc,Quc,Cuc,Vuc,Vbat,Ib,t)
if size(ub,1)==1
    ub=ones(dim,1)*ub;
    lb=ones(dim,1)*lb;
end
...
for i=1:size(SalpPositions,1)
    [Ibat,Ic,Eb,Euc,SalpFitness(1,i)]=fobj(SalpPositions(i,:),Edemand,SOCbat,SOCuc,Quc,Cuc,Vuc,Vbat,Ib,t);
end

%split master and slave groups
...
while l<Max_iter+1
    %split salps into master and slave groups
    [sorted_salps_fitness,sorted_indexes]=sort(SalpFitness);
    n = ceil(numel(sorted_salps_fitness)/2);
    Master_salps = sorted_salps_fitness(1:n);
    Slave_salps =sorted_salps_fitness(n+1:end);
   ...
   %master-slave learning
    c1 = 2*exp(-(4*l/Max_iter)^2); 
    r1 = rand ();
    Lq =(r1*SalpPositions(Master_indexes)-SalpPositions(Slave_indexes))/c1;
