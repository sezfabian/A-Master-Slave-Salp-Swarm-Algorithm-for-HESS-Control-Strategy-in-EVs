...
function [Ibat,Ic,Eb,Euc,Best_score,Best_pos,cg_curve]=DA(N,Max_iter,lb,ub,dim,fobj,Edemand,SOCbat,SOCuc,Quc,Cuc,Vuc,Vbat,Ib,t)
...
X=DAinitialization(N,dim,ub,lb);
Fitness=zeros(1,N);

DeltaX=DAinitialization(N,dim,ub,lb);
...
for iter=1:Max_iter
    ...
    for i=1:N %Calculate all the objective values first
        [Ibat,Ic,Eb,Euc,Fitness(1,i)]=fobj((X(:,i)'),Edemand,SOCbat,SOCuc,Quc,Cuc,Vuc,Vbat,Ib,t);
