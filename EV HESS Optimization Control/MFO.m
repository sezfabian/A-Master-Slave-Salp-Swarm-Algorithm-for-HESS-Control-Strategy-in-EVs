...
function [Ibat,Ic,Eb,Euc,Best_flame_score,Best_flame_pos,Convergence_curve]=MFO(N,Max_iter,lb,ub,dim,fobj,Edemand,SOCbat,SOCuc,Quc,Cuc,Vuc,Vbat,Ib,t)
%display('MFO is optimizing your problem');

%Initialize the positions of moths
Moth_pos=MFOinitialization(N,dim,ub,lb);
...
while Iteration<Max_iter+1
    ...
    for i=1:size(Moth_pos,1)
        ...
        [Ibat,Ic,Eb,Euc,Moth_fitness(1,i)]=fobj(Moth_pos(i,:),Edemand,SOCbat,SOCuc,Quc,Cuc,Vuc,Vbat,Ib,t);  
    ...
    for i=1:size(Moth_pos,1)
        ...
        for j=1:size(Moth_pos,2)
            if i<=Flame_no % Update the position of the moth with respect to its corresponsing flame
                ...
                % Eq. (3.12)
                Moth_pos(i,j)=distance_to_flame*exp(b.*tm).*cos(tm.*2*pi)+sorted_population(i,j);
            end
            ...
            if i>Flame_no % Upaate the position of the moth with respct to one flame
                ...
                % Eq. (3.12)
                Moth_pos(i,j)=distance_to_flame*exp(b.*tm).*cos(tm.*2*pi)+sorted_population(Flame_no,j);
            end
